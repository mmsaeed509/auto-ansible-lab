...
echo -e "${BYellow}[*] Applying additional SSH fixes for Ubuntu...${RESET_COLOR}"

# Additional SSH configuration for Ubuntu to ensure password auth works
sudo bash -c 'echo "PasswordAuthentication yes" > /etc/ssh/sshd_config.d/50-password-auth.conf'
sudo systemctl restart ssh

echo -e "${Green}   ✓ Ubuntu SSH configuration updated${RESET_COLOR}"